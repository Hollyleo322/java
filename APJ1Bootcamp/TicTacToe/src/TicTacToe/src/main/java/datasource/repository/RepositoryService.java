...
public class RepositoryService implements Service {
    final private Repository _repository;
    ...
    public Move getMove(int uuid)
    {
        ...
        minMax(_repository.getCurrentGame(uuid), Constants.AI, result);
        return result;
    }
    ...
    {
        boolean res = false;
        DSGameField currentField = _repository.getCurrentGame(uiid).getGameField();
    ...
    {
        return isWinner(_repository.getCurrentGame(uuid), Constants.AI) || isWinner(_repository.getCurrentGame(uuid), Constants.PLAYER);
    }
    public boolean isTie(int uuid)
    {
        boolean res = true;
        int [][] field = _repository.getCurrentGame(uuid).getGameField().getField();
    ...
    public boolean incorrectData(int uuid)
    {
        ...
        for (int i = 0; i < Constants.ROWSANDCOLS && !res; i++)
        {
            for (int j = 0; j < Constants.ROWSANDCOLS; j++)
            {
                if (field[i][j] != 1 && field[i][j] != 10 && field[i][j] != 0)
                {
                    res = true;
                    break;
                }
