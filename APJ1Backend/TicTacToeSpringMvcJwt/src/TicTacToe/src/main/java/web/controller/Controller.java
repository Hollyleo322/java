...
import domain.model.ComparatorOfCurrentGames;
...
public class Controller{
    ...
    public Controller(DSRepository repository, RepositoryService repositoryService, UserRepository userRepository, JwtProvider jwtProvider) {
    ...
    CurrentGame moveOfSecondPlayer(@RequestBody GameField gameField, @PathVariable int UUID, @RequestHeader("Authorization") String auth) throws UiidNotFoundException, ChangedPreviousMovesException, UiidDoesNotMatchException, IncorrectDataInBodyException, EndOfGame
    {
        ...
        try {
            int id = Integer.parseInt( (String) jwtProvider.getClaims(JwtToken.getToken(auth)).getPayload().get("uuid"));
            cg = repository.findById(UUID).get();
            if (!repository.findById(UUID).get().getUserId().contains(id))
            {
            ...
            if (cg.getCondition().equals(Constants.WAITING_PLAYERS)){
                throw new WaitingForSecondPlayerException();
            }
            if (cg.getCondition().contains(Constants.MOVE_OF_PLAYER)) {
                if (id != getIdFromCondition(cg.getCondition())){
            ...
            else
            {
                ...
                if (repositoryService.isEndOfGame(cg.getUuid()))
                {
                    cg.setCondition(Constants.WIN_OF_PLAYER + id);
                    repository.save(cg);
                    throw new EndOfGame("Player with id " + id + " win");
                }
                if (repositoryService.isTie(UUID)) {
                    cg.setCondition(Constants.TIE);
                    repository.save(cg);
    ...
    String createGameWithId(@PathVariable Integer uuid, @RequestHeader("Authorization") String info)
    {
        ...
        int id = Integer.parseInt( (String) jwtProvider.getClaims(JwtToken.getToken(info)).getPayload().get("uuid"));
        WebCurrentGame currentGame = new WebCurrentGame(uuid, new WebGameField(), Constants.MOVE_OF_PLAYER + id, true, new ArrayList<>(), new Date(System.currentTimeMillis()));
    ...
    String createHumanGameWithId(@PathVariable Integer uuid, @RequestHeader("Authorization") String info) {
        ...
        int id = Integer.parseInt( (String) jwtProvider.getClaims(JwtToken.getToken(info)).getPayload().get("uuid"));
        WebCurrentGame currentGame = new WebCurrentGame(uuid, new WebGameField(), Constants.WAITING_PLAYERS, false, new ArrayList<>(), new Date(System.currentTimeMillis()));
        currentGame.getUserId().add(id);
    ...
    Collection<CurrentGame> getGames() {
        ...
        games.sort(new ComparatorOfCurrentGames());
        return games;
    }
    ...
    public String connect(@PathVariable Integer uuid, @RequestHeader("Authorization") String info) {
        ...
        if (currentGame.getUserId().size() == 2) {
            throw new MaxUsersConnectedException();
        }
        int id = Integer.parseInt( (String) jwtProvider.getClaims(JwtToken.getToken(info)).getPayload().get("uuid"));
        if (currentGame.getUserId().get(0) == id) {
            throw new UiidExistsException(id);
        }
        currentGame.getUserId().add(id);
    ...
    public JwtAuthentication getInfoFromToken(@RequestHeader("Authorization") String token) {
        JwtAuthentication result = null;
        try {
            String accessToken = JwtToken.getToken(token);
            Jws<Claims> claimsJws = jwtProvider.getClaims(accessToken);
            result = JwtUtil.buildAuthentication(claimsJws);
    ...
    @GetMapping("/getFinishedGamesByToken")
    public List<CurrentGame> getFinishedGamesByToken (@RequestHeader("Authorization") String accessToken) {
        accessToken = JwtToken.getToken(accessToken);
        Integer uuid = Integer.parseInt((String)jwtProvider.getClaims(accessToken).getPayload().get("uuid"));
        return repositoryService.getFinishedGamesByUuid(uuid);
    }
    @GetMapping("/getAllFinishedGamesByToken")
    public List<CurrentGame> getAllFinishedGamesByToken (@RequestHeader("Authorization") String accessToken) {
        accessToken = JwtToken.getToken(accessToken);
        Integer uuid = Integer.parseInt((String)jwtProvider.getClaims(accessToken).getPayload().get("uuid"));
        return repositoryService.getAllFinishedGamesByUuid(uuid);
    }
    @GetMapping("/getBestPlayers/{n}")
    public List<StatisticWithName> getBestPlayer(@PathVariable Integer n) {
        List<Statistic> statistics = repositoryService.getBestPlayers(n);
        List<StatisticWithName> result = new ArrayList<>();
        for (Statistic it : statistics) {
            result.add(new StatisticWithName(userRepository.findById(it.getUuid()).get().getLogin() + " " + String.valueOf(it.getUuid()), it.getRatio()));
        }
        result.sort(new ComparatorOfStatistic());
        return result;
    }
    public static int getIdFromCondition(String condition) {
