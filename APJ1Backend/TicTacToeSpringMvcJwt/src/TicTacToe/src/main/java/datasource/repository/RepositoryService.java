...
public class RepositoryService implements Service {
    ...
    public List<CurrentGame> getFinishedGamesByUuid(Integer uuid) {
        return _repository.getSpecificCurrentGames(uuid).stream().map(DataSourceToDomain::convertCurrentGametoDomain).sorted(new ComparatorOfCurrentGames()).toList();
    }
    public List<CurrentGame> getAllFinishedGamesByUuid(Integer uuid) {
        return _repository.getAllFinishedCurrentGames(uuid).stream().map(DataSourceToDomain::convertCurrentGametoDomain).sorted(new ComparatorOfCurrentGames()).toList();
    }

    public List<Statistic> getBestPlayers(Integer n) {
        return _repository.getStatistics(n).stream().toList();
    }
}
